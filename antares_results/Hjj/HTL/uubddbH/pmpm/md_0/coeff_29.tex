\documentclass[varwidth, border=5pt]{standalone}
\usepackage[paperwidth=575cm, paperheight=575cm, margin=1in, landscape]{geometry}
\DeclareMathSizes{1}{1}{1}{0.1}
\usepackage{unicode-math}
\usepackage{mathtools}
\newcommand{\tr}{\text{tr}}
\newcommand{\trfive}{\text{tr5}}
\standaloneenv{my}

\begin{document}
\begin{my}
$\begin{gathered}
\scriptscriptstyle\frac{1[13]⟨2|3+4|1]}{[12]⟨2|1+3|4]} +\\
\scriptscriptstyle\frac{-1[14]⟨2|1+4|3]}{[24]⟨2|1+3|4]} +\\
\scriptscriptstyle\frac{1⟨14⟩[14]⟨4|1+2|3]}{[24]⟨34⟩⟨1|2+4|3]} \phantom{+}
\end{gathered}$
\end{my}
\end{document}
